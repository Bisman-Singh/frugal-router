...
# v45: terse-output contracts. The gate is an INTENT judge — a correct short
# answer scores the same as a correct verbose one, so filler only costs metered
# completion tokens. Accuracy-critical structure is preserved: math/logic keep
# brief working (CoT lifts small-model accuracy on the hard ones remote sees),
# code keeps the full fenced block, summary keeps its content, sentiment keeps
# its label. Caps are lowered to realistic maxima that fit real answers without
# tripping the truncation-retry (which would spend MORE tokens).
_BASE = ("English only. Answer directly with no preamble, no filler, and do not "
         "restate the question.")

CONTRACTS: dict[str, tuple[str, int]] = {
    "factual": (f"{_BASE} Give only the answer itself, in as few words as possible.", 160),
    "math": (f"{_BASE} Show minimal working, then end with 'Answer: <value>' on its own line.", 700),
    "sentiment": (f"{_BASE} Output exactly one label - positive, negative, or neutral - then one short clause.", 90),
    "summarization": (f"{_BASE} Output only the summary itself; obey any stated length or format constraint exactly.", 260),
    "ner": (f"{_BASE} List each entity as 'label: value', one per line and nothing else; labels: person, organization, location, date.", 220),
    "logic": (f"{_BASE} Reason in brief steps checking every constraint, then end with 'Answer: <value>' on its own line.", 800),
    "code_debug": (f"{_BASE} One sentence naming the bug, then the complete corrected code in one fenced block, nothing after it.", 1000),
    "code_gen": (f"{_BASE} Output only the complete, correct, self-contained code in one fenced block.", 1000),
}
